import { IconProps } from '@/interfaces/icon'

export const IExperience: React.FC<IconProps> = ({
  color,
  size = 13,
  ...attributes
}) => {
  return (
    <svg
      version="1.1"
      id="Icons"
      width={30}
      height={30}
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 32 32"
      {...attributes}
    >
      <path
        d="M29.7,18.3c-3.1-3.1-8.2-3.1-11.3,0s-3.1,8.2,0,11.3s8.2,3.1,11.3,0S32.8,21.5,29.7,18.3z M28,25h-4c-0.6,0-1-0.4-1-1v-5
	c0-0.6,0.4-1,1-1s1,0.4,1,1v4h3c0.6,0,1,0.4,1,1S28.6,25,28,25z"
      />
      <path
        d="M14.8,28H2.2l0.1-0.4c0.4-1.8,1.6-3.2,3.4-3.8c2-0.7,3.4-2.5,3.4-4.7c0-0.4-0.2-0.7-0.5-0.9C7,17.4,6,15.6,6,13.8v-2.3
	c2.3,2.2,5.7,3.8,9.7,4.3c-0.4,1.1-1.2,2-2.2,2.6c-0.3,0.2-0.5,0.5-0.5,0.9c0,1.2,0.4,2.3,1.2,3.1c0.7-4.1,3.8-7.3,7.8-8.1
	c0-0.1,0-0.1,0-0.2c0-7.5-4.1-12-11-12S0,6.5,0,14c0,3.2,0.8,6.8,2.1,9.9c-0.9,0.9-1.5,2.1-1.7,3.4l-0.2,0.9c-0.1,0.5,0,0.9,0.3,1.3
	C0.8,29.8,1.2,30,1.7,30H16C15.6,29.4,15.2,28.7,14.8,28z"
      />
    </svg>
  )
}
